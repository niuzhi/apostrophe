...
module.exports = function(self, options) {
  ...
  self.srcset = function(attachment, options) {
    console.log(attachment);
    if (!self.apos.attachments.isSized(attachment)) {
      return '';
    }

    var cropRelationship;
    var maxWidth;

    if (options && options.top && options.left) {
      cropRelationship = options;
    }

    if (options.maxSize) {
      var maxSize = _.find(self.apos.attachments.imageSizes, function (size) {
        return size.name === options.maxSize;
      });
