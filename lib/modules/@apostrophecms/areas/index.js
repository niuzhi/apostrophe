    ...
    return {
      post: {
        async renderWidget(req) {
          ...
          const field = self.apos.schemas.getFieldById(areaFieldId);
  ...
  methods(self, options) {
    return {
      ...
      async renderArea(req, area, context) {
        ...
        const choices = [];
        const field = self.apos.schemas.getFieldById(area._fieldId);
      ...
      async sanitizeItems(req, items, options) {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          try {
            ...
            newItem._id = self.apos.launder.id(item._id) || self.apos.utils.generateId();
          } catch (e) {
            if (Array.isArray(e)) {
      ...
      getBrowserData(req) {
        ...
        _.each(self.widgetManagers, function (manager, name) {
          widgets[name] = (
            manager.options.browser &&
            manager.options.browser.components &&
            manager.options.browser.components.widget
          ) || 'ApostropheWidget';

          widgetEditors[name] = (
            manager.options.browser &&
            manager.options.browser.components &&
            manager.options.browser.components.widgetEditor
          ) || 'ApostropheWidgetEditor';
